...
export function cancelPendingBet({ tickets, balance, index, phase, parlayMode, now, deadline }) {
  ...
  const cancelledIndexes = tickets.flatMap((ticket, ticketIndex) => (
    ticket.placed && ticket.status === "placed" && (parlayMode || ticketIndex === index) ? [ticketIndex] : []
  ));
