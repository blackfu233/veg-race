...
export const RECOVERY_CORE_SCALES: Readonly<Record<string, number>>;
export function recoveryCoreScale(roleIds: string[]): number;
...
export function reserveRecoveryProfit(pool: RecoveryPool, amount: number): { accepted: boolean; amount: number; pool: RecoveryPool };
export function settleRecoveryReservation(pool: RecoveryPool, amount: number, paid: boolean): RecoveryPool;
export function settleRecoveryPool(pool: RecoveryPool, options?: { coreStake?: number; corePayout?: number; actualPayout?: number; releaseActive?: boolean; releasedProfit?: number; thresholdUnit?: number; cooldownUnit?: number; advanceCooldown?: boolean }): RecoveryPool;
