...
import { createRecoveryPool, planRecoveryRelease, recoveryCoreScale, reserveRecoveryProfit, settleRecoveryPool, settleRecoveryReservation } from "../app/recovery-pool.mjs";
...
const coreScaleOverride = numericArgs[1] ? Number.parseFloat(numericArgs[1]) : null;
let coreScale = coreScaleOverride ?? 1;
...
function regularSetup(selectedRoles, ticketRolls) {
  ...
  return { runtimes, targets, wagers, baseRtp: calibrateRoundBaseRtp(wagers) };
}
...
function affordableRegularPlan(plan, selectedRoles, setup, ticketRolls, coreCrash, stake) {
  if (!plan.active) return plan;
  const profitAt = (crashPoint, forceAbility) => Math.max(0, regularPayout(
    selectedRoles,
    setup,
    forceAbility ? ticketRolls.map(forced) : ticketRolls,
    crashPoint,
  ) - stake);
  if (plan.mode === "ability") return profitAt(coreCrash, true) <= plan.available + 1e-9 ? plan : { ...plan, active: false, mode: null, crashFloor: 0 };
  if (profitAt(5, false) > plan.available + 1e-9) return { ...plan, active: false, mode: null, crashFloor: 0 };
  let low = 5;
  let high = plan.crashFloor;
  for (let step = 0; step < 20; step += 1) {
    const midpoint = (low + high) / 2;
    if (profitAt(midpoint, false) <= plan.available + 1e-9) low = midpoint;
    else high = midpoint;
  }
...
function affordableContractPlan(plan, contracts) {
  ...
  const requiredProfit = contracts.reduce((sum, contract) => {
  ...
  return requiredProfit <= plan.available + 1e-9
...
function applyPool(pool, rng, { stake, corePayout, actualPayout, active, assisted }) {
  const next = settleRecoveryPool(pool, {
    ...
    releasedProfit: assisted ? Math.max(0, actualPayout - stake) : 0,
    thresholdUnit: rng(),
    cooldownUnit: rng(),
    advanceCooldown: true,
  });
  if (next.reserve < -1e-9) throw new Error(`Recovery pool overdrawn: ${next.reserve}`);
  return next;
}
...
function simulateRegular(selectedRoles, rng) {
  ...
  for (let entry = 0; entry < samples; entry += 1) {
    ...
    const coreCrash = crashPointFromUnit(rng(), setup.baseRtp * coreScale);
    const plan = affordableRegularPlan(planRecoveryRelease(pool, stake, rng()), selectedRoles, setup, ticketRolls, coreCrash, stake);
...
function simulateContracts(selectedRoles, rng) {
  ...
  for (let entry = 0; entry < samples; entry += 1) {
    ...
    const baseRtp = calibratePumpkinContracts(initial);
    let actual = initial.map((contract) => ({ ...contract, baseRtp, assisted: false }));
    let core = initial.map((contract) => ({ ...contract, baseRtp }));
    while (actual.some((contract) => contract.active)) {
      const exposure = actual.filter((contract) => contract.active).length;
      const coreCrash = crashPointFromUnit(rng(), baseRtp * coreScale);
      ...
      actual = actual.map((contract, index) => {
        ...
        const newlyAssisted = plan.active && !contract.poolAssisted && actualCrash > coreCrash + 1e-9;
        ...
        if (newlyAssisted) {
          ...
          const reservation = reserveRecoveryProfit(pool, Math.max(0, finalPayout - contract.stake));
        ...
        if (!actualResult.contract.active) {
          ...
          assisted ||= nextAssisted && actualResult.payout > coreResult.payout;
      ...
      totalPayout += actualPayout;
      pool = applyPool(pool, rng, { stake: settledStake, corePayout, actualPayout, active: releaseApplied, assisted: assisted && !actual.some((contract) => contract.poolReserved > 0) });
    }
  }
...
function simulateConfiguration(selectedRoles) {
  if (!calibrateMode && coreScaleOverride === null) coreScale = recoveryCoreScale(selectedRoles);
...
if (calibrateMode) {
  const scales = Object.fromEntries(selectedConfigurations.map((selectedRoles) => {
    let low = .3;
    let high = 1;
    let best = { scale: high, deviation: Infinity, rtp: 0 };
    for (let pass = 0; pass < 10; pass += 1) {
      coreScale = (low + high) / 2;
      const result = simulateConfiguration(selectedRoles);
      const deviation = Math.abs(result.rtp - TARGET_RTP);
      if (deviation < best.deviation) best = { scale: coreScale, deviation, rtp: result.rtp };
      if (result.rtp < TARGET_RTP) low = coreScale;
      else high = coreScale;
    }
    const key = [...selectedRoles].sort().join("|");
    console.log(`${key.padEnd(17)} scale ${best.scale.toFixed(6)} · ${(best.rtp * 100).toFixed(3)}%`);
    return [key, Number(best.scale.toFixed(6))];
  }));
...
const results = selectedConfigurations.map((selectedRoles) => {
  ...
  console.log(`${label.padEnd(13)} ${(result.rtp * 100).toFixed(3)}% · pool ${(result.pool.reserve / selectedRoles.length).toFixed(2)}× · releases ${result.pool.releaseCount}`);
  return { label, roles: selectedRoles, rtp: result.rtp, pool: result.pool, ability: selectedRoles.length === 2 ? describeDuoPair(selectedRoles).summary : "單角能力" };
});
...
console.log(JSON.stringify({ samplesPerConfiguration: samples, configurations: results.length, coreScaleMode: coreScaleOverride ?? "per-combination", seedTag, meanRtp: weightedRtp, maxDeviation, targetRtp: TARGET_RTP }, null, 2));
}
